# Dto generator
`dtogen_cli` is a command-line app which is used to generate DTOs
...
```sh
dtogen_cli -i shelf.json -o shelf.dart -p CreateShelf --init-class Shelf --no-from-json --no-to-entity 
```
